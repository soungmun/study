...
- **서버 점검 모드** (관리자 전용 토글) — ON 시 ① 관리자 외 모든 `/api/**` 호출을 503으로 차단 ② 가입 회원 전체(이메일 보유자)에게 점검 안내 메일 1회 비동기 발송. 차단 예외 경로: `/api/admin/**`, `/api/auth/login|logout|me`, 소셜 로그인 콜백
...
| POST | `/api/admin/maintenance/disable` | 관리자 | 점검 모드 OFF |
