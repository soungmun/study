...
- 네이버 OAuth2 로그인 (Authorization Code + state CSRF 검증, 서버 사이드)
- 구글 OAuth2 로그인 (Authorization Code + state CSRF 검증, 서버 사이드)
- 회원정보 수정 (`/me/edit`): 닉네임·이메일·비밀번호 변경
- 비밀번호 찾기 / 재설정 (이메일 토큰, 30분 유효)
- 가입·비밀번호 변경 시 알림 메일 자동 발송 (네이버/Gmail SMTP)
- 회원가입 이메일 인증 (6자리 코드, 5분 유효) — 코드 발송 전 **메일함 존재 확인**(DNS MX 조회 + SMTP RCPT). 없는 주소면 발송을 막고 "없는 메일 주소입니다" 안내. 단, 발신 서버 아웃바운드 25번 포트가 막혀 있으면 검증 불가로 판단해 그대로 발송 (`MailboxVerifier`)
...
**관리**
- 회원 목록 조회 (로그인 필요)
- **서버 점검 모드** (관리자 전용 토글) — ON 시 ① 관리자 외 모든 `/api/**` 호출을 503으로 차단 ② 가입 회원 전체(이메일 보유자)에게 점검 안내 메일 1회 비동기 발송. OFF 시에도 가입 회원 전체에 **점검 완료 안내 메일** 1회 비동기 발송. 차단 예외 경로: `/api/admin/**`, `/api/auth/login|logout|me`, 소셜 로그인 콜백
...
| 구분 | 기술 |
|------|------|
| Backend | Java 17, Spring Boot 4.0.6, Spring Data JPA, Spring Validation, RestClient, springdoc-openapi (Swagger), Spring Mail |
| Database | MariaDB |
...
| naverId | String(100) | UNIQUE | 네이버 사용자 ID |
| googleId | String(100) | UNIQUE | 구글 사용자 ID (`sub`) |
| username | String(50) | UNIQUE | 일반 가입 아이디 |
| password | String(100) | - | BCrypt 해시 (소셜 로그인 가입자는 null) |
| nickname | String(100) | - | 표시명 |
| profileImage | String(500) | - | 카카오/네이버/구글 프로필 이미지 URL |
...
│   ├── controller/        # REST API
│   │   ├── AuthController.java          # 로그인/로그아웃/me/카카오/네이버/구글
...
│   ├── service/           # AuthService, KakaoOAuthService, NaverOAuthService, GoogleOAuthService,
│   │                      # EmailService, NoticeService, BookService, PlaceService,
...
| `naver.oauth.client-id`, `client-secret`, `redirect-uri` | 네이버 OAuth 설정 (환경변수 `NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` / `NAVER_REDIRECT_URI`) |
| `google.oauth.client-id`, `client-secret`, `redirect-uri` | 구글 OAuth 설정 (환경변수 `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_REDIRECT_URI`) |
| `app.frontend.url` | 카카오/네이버/구글 콜백 후 리다이렉트할 프론트 URL |
...
>
> ⚠️ **네이버 개발자센터 활성화 필요** ([developers.naver.com/apps](https://developers.naver.com/apps))
> - 앱 등록 후 **API 설정 → 사용 API → 네이버 로그인** 추가
> - 권한 동의 항목: 회원이름 / 이메일 / 닉네임 / 프로필사진
> - **로그인 오픈 API 서비스 환경 → PC웹** 추가
>   - 서비스 URL: `http://localhost:5173`
>   - Callback URL: `http://localhost:8080/api/auth/naver/callback`
>
> ⚠️ **Google Cloud Console 활성화 필요** ([console.cloud.google.com/apis/credentials](https://console.cloud.google.com/apis/credentials))
> - **사용자 인증 정보 → OAuth 2.0 클라이언트 ID** (애플리케이션 유형: 웹 애플리케이션) 생성
> - **승인된 자바스크립트 원본**: `http://localhost:5173`
> - **승인된 리디렉션 URI**: `http://localhost:8080/api/auth/google/callback`
> - OAuth 동의 화면 범위: `openid`, `email`, `profile`
> - 생성된 클라이언트 ID/Secret 을 `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` 환경변수로 주입
...
| GET  | `/api/auth/naver/login` | - | 네이버 인가 페이지로 리다이렉트 (`returnTo` 지원, CSRF 방지용 state 발급) |
| GET  | `/api/auth/naver/callback` | - | 네이버 콜백 (state 검증 → 토큰 → 사용자 → 세션 등록) |
| GET  | `/api/auth/google/login` | - | 구글 인가 페이지로 리다이렉트 (`returnTo` 지원, CSRF 방지용 state 발급) |
| GET  | `/api/auth/google/callback` | - | 구글 콜백 (state 검증 → 토큰 → 사용자 → 세션 등록) |
...
#### `/api/auth/me` 응답 (200)
```json
{ "id": 1, "kakaoId": null, "naverId": null, "username": "alice", "nickname": "앨리스",
...
```
- 카카오/네이버/구글 가입자(비번 없음)는 비밀번호 변경 불가 → `400 KAKAO_ONLY`
...
| POST   | `/api/notices` | 등록 (**관리자만**, 미로그인 401 / 비관리자 403) |
...
> 등록은 **관리자(`app.admin.username`)만** 가능합니다. 수정/삭제는 **작성자 본인 또는 관리자**만 가능합니다(상세 응답의 `canEdit` 플래그로 프론트가 수정·삭제 버튼 노출 제어). 목록의 글쓰기 버튼은 `GET /api/admin/me`로 관리자일 때만 노출됩니다.
...
| GET  | `/api/admin/me` | 세션 | 현재 사용자가 관리자인지 |
| GET  | `/api/admin/maintenance/status` | - | 점검 모드 상태 (공개 — 배너 표시용) |
| POST | `/api/admin/maintenance/enable` | 관리자 | 점검 모드 ON + 전체 회원에 안내 메일 비동기 발송 |
| POST | `/api/admin/maintenance/disable` | 관리자 | 점검 모드 OFF + 전체 회원에 완료 안내 메일 비동기 발송 |
...
| `/admin/broadcast` | 서버 점검 모드 토글 | 관리자만 |
