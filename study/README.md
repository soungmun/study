...
- **장소 검색 + 지도**: 카카오 키워드 검색 + Kakao Maps SDK로 지도 표시
  ...
  - **선택한 장소의 현재 날씨 표시**: Open-Meteo API로 기온/풍속/풍향/날씨 상태(아이콘) 조회
  - **선택한 장소의 미세먼지 표시**: Open-Meteo Air Quality API로 PM10/PM2.5/유럽 AQI 조회 + 등급(좋음/보통/나쁨/매우나쁨) 색상 표시
- **회원정보 수정**: `/me/edit`에서 닉네임/이메일/비밀번호 변경
  - 일반 가입자: 현재 비밀번호 검증 후 새 비밀번호로 변경
  - 카카오 가입자: 닉네임/이메일만 수정 가능 (비밀번호 변경 불가)
- **이메일 발송 / 비밀번호 찾기**:
  - 회원가입 시 이메일 **필수** 입력 → 환영 메일 자동 발송 (비동기)
  - `/forgot`에서 이메일 입력 → 30분 유효 토큰을 담은 재설정 링크 메일 발송
  - 메일 링크(`/reset?token=…`)에서 새 비밀번호 설정
  - 회원정보 수정에서 비밀번호 변경 시 변경 알림 메일 발송
  - SMTP는 환경변수로 전환 가능 (기본: 네이버 SMTP)
    - **네이버**: `MAIL_USERNAME=<id>@naver.com`, `MAIL_PASSWORD=<로그인 비밀번호>` (네이버 메일 환경설정 → POP3/IMAP → SMTP 사용함)
...
| 외부 API | Kakao 책 검색 (`/v3/search/book`), 카카오 로컬 키워드 검색 (`/v2/local/search/keyword.json`), Kakao Maps JavaScript SDK, Open-Meteo (날씨), Open-Meteo Air Quality (대기질) |
| 기타 | Lombok, Gradle |
