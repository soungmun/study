...
| profileImage | String(500) | - | 카카오/네이버 프로필 이미지 URL |
...
│   ├── controller/        # REST API
│   │   ├── AuthController.java          # 로그인/로그아웃/me/카카오/네이버/네이버
...
│   ├── service/           # AuthService, KakaoOAuthService, NaverOAuthService,
...
| `app.frontend.url` | 카카오/네이버 콜백 후 리다이렉트할 프론트 URL |
...
```
- 카카오/네이버 가입자(비번 없음)는 비밀번호 변경 불가 → `400 KAKAO_ONLY`
